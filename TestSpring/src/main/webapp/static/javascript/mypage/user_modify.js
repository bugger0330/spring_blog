...
function onload(ss){
	inputItems[0].value = ss.username;
	inputItems[1].value = ss.name;
	inputItems[2].value = ss.email;
	inputItems[3].value = ss.phone;
	inputItems[4].value = ss.nickname;
}
...
password.onblur = () => {
	...
	submitBtn.onclick = () => {
		if(password.value == "" && flag == false){
			...
			$.ajax({
				type : "put",
				url : "/app/mypage/user-modify/update",
				data : JSON.stringify({
					username : inputItems[0].value,
					nickname : inputItems[4].value,
					email : inputItems[2].value,
					phone : inputItems[3].value,
